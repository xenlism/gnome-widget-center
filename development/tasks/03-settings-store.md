...
(widget ไม่ยุ่งกับ filesystem เอง) ตามที่ตัดสินใจไว้ใน `development/docs/ARCHITECTURE.md` §2.3 และ
`development/docs/SETTINGS_SPEC.md`

**หมายเหตุ (แก้ไข 2026-07-13):** เวอร์ชันก่อนหน้าของไฟล์นี้เขียนผิดพลาดว่าให้ใช้ DBus +
SQLite service แยกโปรเซส ซึ่ง**ขัดกับ `development/docs/ARCHITECTURE.md`/`development/docs/SETTINGS_SPEC.md` โดยตรง**
(ดูเหตุผลเต็มใน `development/docs/SETTINGS_SPEC.md` หัวข้อ "ทำไมไม่ใช้ GSettings สำหรับ widget" — หลักการ
...
อ่าน `development/docs/SETTINGS_SPEC.md` ทั้งไฟล์ก่อนเริ่ม — เป็นสเปกเดียวที่ต้องอ้างอิงสำหรับ task นี้
...
- Host-level settings (widget ไหน enable อยู่ ฯลฯ): `~/.config/gnome-widget-center/host.json`
  หรือ GSettings ของ core เองก็ได้ (`products/extension/schemas/....gschema.xml`) — เป็นของ core ไม่ใช่
...
- `products/extension/lib/storageService.js` — มี `getWidgetSettings`/`saveWidgetSettings` แบบ synchronous
  อยู่แล้ว แต่ยังไม่มี default-merge, ไม่มี debounce, ไม่มี proxy auto-save, ไม่มี path sanitize
- `products/extension/lib/settingsService.js` — จัดการเฉพาะ GSettings ของ host เอง (ของเดิมถูกต้องแล้ว
  ตามหลักการ ไม่ต้องแก้ในทีนี้ นอกจากเชื่อมกับ `widgetSettings.js` ใหม่)

## Files to touch

- `products/extension/lib/widgetSettings.js` (สร้างใหม่ — ตาม class `WidgetSettings` ที่ระบุใน
  `development/docs/SETTINGS_SPEC.md` หัวข้อ "Host-side: WidgetSettings class") — **ทำแล้ว**:
...
- `products/extension/lib/storageService.js` — **ทำแล้ว**: เพิ่ม `_sanitizeWidgetId()`,
  ...
  `development/docs/SETTINGS_SPEC.md` กำหนด (`widgets/<id>.json`) — แก้ให้สร้างโฟลเดอร์ย่อย `widgets/`
  ใน `init()` แล้วอ่าน/เขียนที่ `widgets/<id>.json` ตรงสเปกแล้ว
- `products/extension/schemas/org.gnome.shell.extensions.widget-center.gschema.xml` — **ทำแล้ว**
...
- `products/extension/lib/settingsService.js` — **ทำแล้ว**: แก้ให้ใช้ `Extension.getSettings()`
  แทน `Gio.SettingsSchemaSource.get_default()` (ของเดิม throw เสมอเพราะมองหาแค่ schema
  ที่ติดตั้งระดับระบบ — ดู `development/docs/SETTINGS_SPEC.md` "Host settings เอง")
- `products/extension/lib/widgetLoader.js` — **ทำแล้ว (2026-07-14)**: constructor รับ
...
- `products/extension/extension.js` — **ทำแล้ว (2026-07-14)**: ส่ง `this._storage` เข้า
...
4. เชื่อมกับ `extension.js`: ตอนโหลด widget แต่ละตัว เรียก
   ...
   `api.settings` เข้า constructor ของ widget (อ้างอิง data flow ใน `development/docs/ARCHITECTURE.md` §4)
...
1. **Two-phase load แทน `load(widgetId, defaults)` ตัวเดียวตามที่ spec เขียนไว้** — เจอปัญหา
   ...
   api.settings` ตาม `development/docs/WIDGET_API.md` §3) แต่ defaults มาจาก
...
1. **`settingsService.js` เดิม throw เสมอตอน `init()`** — เรียก
   ...
   ไม่ควรมีตามเป้าหมายโปรเจกต์) — ไม่เคยมีไฟล์ `.gschema.xml`/`products/extension/schemas/` อยู่ด้วยซ้ำ
   แก้โดย: (a) สร้าง `products/extension/schemas/org.gnome.shell.extensions.widget-center.gschema.xml`
...
- `products/extension/lib/widgetSettings.js` — class `WidgetSettings` ตาม `SETTINGS_SPEC.md` (load +
