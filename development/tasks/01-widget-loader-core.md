...
อ่าน `development/docs/WIDGET_API.md` ทั้งหมด — ไฟล์นี้คือสัญญาที่ `widgetLoader.js` ต้อง enforce
...
- `products/extension/lib/widgetLoader.js` (สร้างใหม่)
- `products/extension/extension.js` (แก้ให้เรียกใช้ loader ตอน enable/disable)
- `products/extension/widgets/_template/` (สร้าง widget เปล่า ๆ ไว้ทดสอบ loader — ยังไม่ต้องมี UI จริง)
...
4. `products/extension/widgets/_template/` ให้มี `metadata.json` + `widget.js` ที่ implement ครบตาม WIDGET_API.md
...
- [ ] วาง widget ใหม่ (copy จาก `_template`) ในโฟลเดอร์ user-installed แล้ว rescan โดย**ไม่แก้ไฟล์ใน
      `products/extension/` เลยสักบรรทัด** widget ใหม่ถูกโหลดขึ้นมา
...
## Real hardware verification (2026-07-16)

**สถานะ: ยืนยันบนเครื่องจริงบางส่วนแล้ว** — ได้ log จริงจาก `journalctl` ครั้งแรก:

```
[widget-loader] loaded "my-widget" from .../gnome-widget-center@xenlism.github.io/widgets/_template
[widget-loader] loaded "clock" from .../gnome-widget-center@xenlism.github.io/widgets/clock
[widget-loader] loaded "media-player" from .../gnome-widget-center@xenlism.github.io/widgets/media-player
[widget-center] loaded 3 widget(s)
```

**ยืนยันแล้วจริง:** `discover()` + `loadAll()` ทำงานจบครบบน GNOME Shell จริงไม่ crash, ทั้ง
`clock` และ `media-player` (task 06) โหลดสำเร็จ, extension enable ผ่านจนจบ (บรรทัด D-Bus
`firefox.desktop` "ServiceUnknown" ใน log เดียวกันเป็นของ GNOME Shell's search provider เอง
ไม่เกี่ยวกับ widget-center)

**บั๊กที่พบ (แก้แล้ว 2026-07-16):** `widgets/_template/` (โฟลเดอร์ scaffold สำหรับ third-party
dev คัดลอกไปทำ widget ใหม่ — ดู `development/docs/PUBLISHING_A_WIDGET.md`) ถูก `discover()` สแกนเป็น
widget จริงด้วย เพราะ `metadata.json` ของมันมี `id: "my-widget"` ที่ผ่าน validation ปกติทุก
ประการ — ไม่มีอะไรใน loop เดิมที่ข้ามโฟลเดอร์ที่ขึ้นต้นด้วย `_` เลย ผลคือ template ถูกนับเป็น
widget ที่ 3 จริง (ตรงกับ log `loaded 3 widget(s)` — ที่ถูกต้องควรเป็น 2: `clock` +
`media-player` เท่านั้น)

**แก้แล้ว:** เพิ่มเงื่อนไขข้าม folder ที่ชื่อขึ้นต้นด้วย `_` ใน `discover()` ก่อนจะไปถึงขั้น
อ่าน `metadata.json` เลย (ข้ามแบบเงียบ ไม่นับเป็น error ใน `this.errors` เพราะไม่ใช่ widget
ที่พัง แค่ไม่ใช่ widget) `node --check` ผ่าน ยังไม่ได้ re-run บนเครื่องจริงเพื่อยืนยันว่า log
เหลือแค่ 2 บรรทัด `loaded ...` — รอ session ถัดไปที่มีคน deploy โค้ดใหม่ไปทดสอบ

**ยังต้องทำต่อ:** ข้อ 1-2 ใน "ยังไม่ยืนยัน" ด้านบน (rescan user-installed widget,
`disable()` กลางคัน) ยังไม่มี evidence จาก log นี้ — log ที่ได้มาเป็นแค่ enable ตอน shell
เริ่มต้นปกติ ยังไม่ครอบคลุมสถานการณ์เหล่านั้น
...
- Acceptance criteria ข้อแรก ("วาง widget ใหม่ใน user-installed แล้ว rescan โดยไม่แก้ไฟล์
  `products/extension/`") — logic รองรับอยู่แล้ว (path `~/.local/share/gnome-widget-center/widgets/`
...
1. คัดลอก `products/extension/*` ทับของเดิมที่ `~/.local/share/gnome-shell/extensions/gnome-widget-center@local.dev/`
2. คัดลอกโฟลเดอร์ `products/extension/widgets/_template/` ไปเป็น
   `~/.local/share/gnome-widget-center/widgets/my-test-widget/` (จำลอง user-installed widget)
   แล้วแก้ `id` ใน `metadata.json` ให้ไม่ชนกับตัวที่ bundle มาด้วย `products/extension/widgets/` (ถ้ามี)
...
5. เติมผลจริงตรงนี้ แล้ว tick `- [x]` ที่ `development/tasks/ROADMAP.md`
