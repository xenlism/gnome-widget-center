...
ข้อเสีย/ข้อจำกัดที่ต้องรับรู้และ validate ใน Day 1 (`development/tasks/00-project-setup.md`):
...
widget ใหม่ = สร้างโฟลเดอร์ใหม่ตามสเปกใน `development/docs/WIDGET_API.md` แล้ววางไว้ในพาธข้างบน
**ไม่ต้องแตะไฟล์ใน `products/extension/` เลย**
...
เก็บแยกเป็นของ core เอง ใน `products/extension/schemas/...gschema.xml` (อันเดียวที่ยัง compile
เป็น GSettings จริง เพราะเป็นของ core ไม่ใช่ของ third-party)

### 2.4 Control Center (GUI จัดการ widget)

`products/extension/prefs.js` เป็น GTK4/Libadwaita window แสดง list widget ที่ค้นพบทั้งหมด
