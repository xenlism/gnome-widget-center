...
ของตัวเอง แล้วแจกให้คนอื่นติดตั้งได้ ไม่ต้องอ่านโค้ดใน `products/extension/` เลย — อ่านแค่เอกสารนี้
กับ [`development/docs/WIDGET_API.md`](WIDGET_API.md) (สเปกเต็มของสิ่งที่ widget หนึ่งตัว "ต้อง" ทำ/ห้ามทำ)
...
1. **Copy** โฟลเดอร์ `products/extension/widgets/_template/` ทั้งโฟลเดอร์ ไปเป็นโฟลเดอร์ใหม่ชื่อ widget ของคุณ
...
3. **วางโฟลเดอร์** ไว้ที่:
   ...
   นอกเหนือจาก widget ที่มากับตัวโปรเจกต์ ดู `development/docs/ARCHITECTURE.md` §2.2)
...
**ระหว่างพัฒนา** ถ้าเปิด dev-mode ไว้ (ดู `development/tasks/08-hot-reload-dev-mode.md`) การแก้ไฟล์ของ
...
ของ [`development/docs/WIDGET_API.md`](WIDGET_API.md)** ก่อนเสมอ (เช่น `buildActor()` ห้าม throw แม้
...
- [ ] ถ้า widget มี `stylesheet.css`: **รู้ข้อจำกัดปัจจุบัน** — host ยังไม่โหลด `stylesheet.css`
      เข้า theme context ให้อัตโนมัติ (ดู comment ในไฟล์ตัวอย่างของ `products/extension/widgets/clock/`) ต้องพึ่ง
...
- `api-version` **ต้องตรงกับ** เวอร์ชันของ `WidgetAPI` ที่ host เวอร์ชันที่ผู้ใช้ติดตั้งอยู่รองรับ
  (ปัจจุบันคือ `1` — ดู `development/docs/WIDGET_API.md` §2) ถ้า host เปลี่ยน API แบบ breaking ในอนาคต
...
- ถ้า widget ใช้ฟีเจอร์ของ GNOME Shell เวอร์ชันใหม่กว่าขั้นต่ำที่ `development/docs/WIDGET_API.md` §6
  ระบุไว้ (GNOME Shell 45+) ให้ระบุ `shell-version` เพิ่มใน `metadata.json` ของ widget ด้วย

## เอกสารที่เกี่ยวข้อง

- [`development/docs/WIDGET_API.md`](WIDGET_API.md) — สเปกเต็มของ `widget.js`/`prefs.js`/`WidgetAPI`
- [`development/docs/ARCHITECTURE.md`](ARCHITECTURE.md) — ทำไม widget ถึงเป็นโฟลเดอร์ปลั๊กอินอิสระ
  ไม่ต้องแก้ core (§2.2)
- [`development/docs/SETTINGS_SPEC.md`](SETTINGS_SPEC.md) — รูปแบบไฟล์ settings ที่ `api.settings` ผูกอยู่
- `products/extension/widgets/_template/` — โฟลเดอร์ตั้งต้นที่ใช้ใน Quick start ข้อ 1
