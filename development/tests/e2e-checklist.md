# development/tests/e2e-checklist.md — End-to-end regression (tasks 00-09)

รวบยอด acceptance criteria ของทุก task 00-09 ไว้ในที่เดียว ตามที่ `development/tasks/10-testing-release.md`
...
`development/tasks/ROADMAP.md` และ Notes from implementation ของแต่ละ task ตั้งแต่ 01 เป็นต้นมา) คอลัมน์
...
- ✅ `gnome-extensions enable` ใช้งานได้บน GNOME 50 Wayland จริง — ยืนยันแล้วโดยผู้ใช้จริงบน
  เครื่อง (ดู "Notes from implementation" ของ `development/tasks/00-project-setup.md`)
...
- ⚠️ วาง widget ใหม่ในโฟลเดอร์ user-installed โดยไม่แก้ `products/extension/` แล้วถูกโหลด — โค้ด
...
- ⚠️ widget สอง id ซ้ำกัน → ตัวที่สอง skip พร้อม log — ต้องอ่านโค้ด `discoverWidgets()` ยืนยันว่า
  reject ตาม id ซ้ำจริง (`development/docs/WIDGET_API.md` §2 พูดถึงพฤติกรรมนี้) แต่ยังไม่รันจริง
...
- ✅ เขียนไฟล์ settings ภายใน ~300ms, merge defaults, sanitize path traversal, สอง widget ไม่ชน
  ...
  (ดู "Notes from implementation" ของ `development/tasks/03-settings-store.md`) — ระดับ logic ผ่านแน่นอน
...
- ⚠️ Control Center เห็น widget ทุกตัว (bundled + user), toggle ปิดแล้วหายจากพื้นโต๊ะทันที,
  ...
  `products/extension/prefs.js`/`prefsWidgetList.js` แล้วตรงสเปกทุกข้อ (มี error section แยกจริง,
...
- ⚠️ known limitation ที่ comment ไว้ใน `products/extension/prefs.js`: การแก้ settings ผ่าน Control
...
- ⚠️ `media-player`: แสดงชื่อเพลง/ศิลปินจาก MPRIS ภายในไม่กี่วินาที, ปุ่ม Play/Pause/Next/Previous
  ...
  Notes from implementation ของ `development/tasks/06-widget-sdk-example.md`) จำเป็นต้องมี media player
  จริงที่รองรับ MPRIS (Spotify/VLC/Firefox) บนเครื่องที่มี GNOME Shell เพื่อยืนยัน
- ✅ `development/docs/WIDGET_API.md` §8 ถูกเพิ่มจริงแล้ว — ตรวจสอบแล้วมีอยู่จริงในไฟล์

## Task 07 — Multi-monitor Support

- ❓ ถอดจอรอง → widget ย้ายมาจอหลัก, เสียบจอเดิมกลับ → พฤติกรรมตามที่ตัดสินใจ, HiDPI ไม่เพี้ยน —
  **ไฟล์ `development/tasks/07-multi-monitor-support.md` ไม่มี section "Notes from implementation" เลย**
  แม้ `products/extension/lib/monitorWatcher.js` จะมีอยู่จริงและถูก import/wire เข้า `extension.js`
  แล้ว (`this._monitors = new MonitorWatcher()`) — สถานะจริงไม่ชัดเจนว่า "เสร็จ" ตามคำนิยามของ
  `development/tasks/CONTRIBUTING.md` (ต้องมี Notes ท้ายไฟล์ตอนทำเสร็จ) หรือยังทำค้างอยู่ — **ควรเปิด task
...
- ❓ เช่นเดียวกับ task 07 — **`development/tasks/08-hot-reload-dev-mode.md` ไม่มี section "Notes from
  implementation"** แม้ `products/extension/lib/devWatcher.js` จะมีอยู่จริงและถูก wire เข้า
...
- ✅ `development/docs/PUBLISHING_A_WIDGET.md` เขียนครบตาม Quick start/แจกจ่าย/checklist/versioning,
  `widgets/_template/` (ทั้ง 2 ชุด — ดูหมายเหตุ duplicate ใน Notes from implementation ของ
  `development/tasks/09-packaging-third-party-docs.md`) มี TODO ครบ, `node --check` ผ่านทุกไฟล์ที่แก้ —
...
- ✅ `products/extension/extension.js` ไม่ import `Gtk` เลย (ตรวจด้วย `grep "^import" products/extension/extension.js`)
- ✅ ไม่มี custom `constructor()` ใน `extension.js` ที่ทำงานหนัก/spawn subprocess — ใช้ default
  จาก `Extension` base class
- ✅ ไม่มี `metadata.json` ไฟล์ไหนใน `products/extension/` หรือ widget bundled ประกาศ `"session-modes"`
  (ยืนยันด้วย `grep -r session-modes` ไม่เจอผลเลย) — ตรงกับการตัดสินใจใน
  `development/tasks/00-project-setup.md` ที่ต้องการให้กลไก auto-disable-on-lock ของ GNOME ทำงานแทน
...
1. รันบนเครื่องที่มี GNOME Shell 45+ จริง (แนะนำ GNOME 50 / Wayland ตามที่ project กำหนดไว้
   หลักใน `development/docs/ARCHITECTURE.md`) แล้วไล่ทำตาม checklist นี้ทีละข้อ แก้ ✅/⚠️/❓ เป็นผลจริง
2. ~~เติม "Notes from implementation" ให้ `development/tasks/07-multi-monitor-support.md` และ
   `development/tasks/08-hot-reload-dev-mode.md`~~ **แก้แล้ว (2026-07-16)**
3. ~~ตัดสินใจเรื่อง top-level `widgets/` vs `products/extension/widgets/`~~ **ตัดสินใจแล้ว
   (2026-07-16)** — `products/extension/widgets/` เป็นที่เดียว ไม่มี top-level `widgets/` อีก
   ดู `development/tasks/ROADMAP.md`'s "Decision (2026-07-16)"
