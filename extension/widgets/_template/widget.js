...
// core files need to change. See docs/PUBLISHING_A_WIDGET.md for the full
...
export default class TemplateWidget {
    /**
     * @param {WidgetAPI} api - see docs/WIDGET_API.md §5. This is the real
     ...
     *   write, see docs/SETTINGS_SPEC.md), api.logger is pre-tagged with
    ...
    buildActor() {
        ...
        // (extension/widgets/clock, extension/widgets/media-player) for
    ...
    // docs/WIDGET_API.md §8 for the DBus/MPRIS pattern specifically).
    enable() {
        ...
        // guidelines (linked from docs/WIDGET_API.md §3) explicitly flag.
    ...
    // that also calls disable()/enable() - see docs/WIDGET_API.md §3).
    disable() {
        if (this._timeoutId) {
    ...
    // missing - see docs/SETTINGS_SPEC.md). Keep every key your widget.js
