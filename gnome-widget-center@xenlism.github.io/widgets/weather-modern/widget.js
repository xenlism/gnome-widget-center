...
// Same visual family as weather-minimal/weather-dark, but adds two
// things those don't have: a user-configurable refresh rate
// ("refreshMinutes") and an alpha-enabled background color (drag the
// color dialog's alpha slider for a translucent/transparent card).
//
...
// "13.756331,100.501762"), config.json's `fieldType: "location"` (see
// WIDGET_API.md §6.4) - a validated text entry with a built-in
// IP-detect suffix button, identical UI/behavior across all three
// weather widgets (widgetConfigUI.js's _locationRow(), 2026-07-31, see
// PROJECT_STATUS.md). Replaces this widget's own earlier "Location" text
// field + separate momentary "Detect automatically" switch, and, before
// that, weather-dark/weather-minimal's original two-field "Place"
// autocomplete + "Location" pair (2026-07-29 fix, PROJECT_STATUS.md):
// that autocomplete row only saved on Enter/selecting a suggestion, so
// typing and tabbing away silently didn't persist anything.
...
export default class WeatherPanelWidget {
    ...
    // (typing a new "location", or the settings page's own location
    // button resolving one from IP - see WIDGET_API.md §6.4's
    // `fieldType: "location"` - shouldn't wait for the next scheduled
    // tick), and re-arm the timer if the refresh rate changed.
    onSettingsChanged() {
        this._render();
     ...
     * settings page's location field (including its IP-detect button) writes) -
