...
export default class MediaPlayerSquareWidget {
    ...
    _launchApp() {
        ...
        try {
            ...
            if (appInfo) appInfo.launch([], null); else this._api.logger.info(`media-player-square: could not read .desktop file at ${path}`);
        } catch (e) {
            this._api.logger.info(`media-player-square: failed to launch ${path}: ${e}`);
        }
    }
    _applyArtStyle() {
        if (this._artIcon) this._artIcon.set_style(`border-radius: ${COVER_RADIUS}px; background-size: cover; background-position: center;`);
    }
    _render() {
        ...
        this._titleLabel.set_style(`color: ${infoColor}; font-family: ${infoFont.family}; font-size: ${infoFont.size}px; font-weight: bold;`);
        this._albumLabel.set_style(`color: ${infoColor}; font-family: ${infoFont.family}; font-size: ${Math.max(8, infoFont.size - 3)}px; opacity: 0.85;`);
        this._artistLabel.set_style(`color: ${infoColor}; font-family: ${infoFont.family}; font-size: ${Math.max(8, infoFont.size - 3)}px; opacity: 0.85;`);
        ...
        for (const button of [ this._prevButton, this._playPauseButton, this._nextButton ]) {
            button.set_style(`background-color: rgba(0,0,0,0.45); border-radius: 999px; padding: 8px; color: ${buttonColor};`);
    ...
    _renderState(state) {
        ...
        if (state.artUrl.length > 0) {
            try {
                ...
                this._artIcon.set_style(`border-radius: ${COVER_RADIUS}px; background-size: cover; background-position: center; ` + `background-image: url("${file.get_uri()}");`);
                this._showArt();
            } catch (e) {
                this._showFallbackArt();
            }
