...
export default class GeekDateWeekBigWidget {
    constructor(api) {
        this._api = api;
    ...
    buildActor() {
        this._layers = createLayeredCard({
            contentStyleClass: "geek-date-week-big-widget-root"
        });
        ...
        this._topLabel = new St.Label({
            style_class: "geek-date-week-big-widget-top",
            x_expand: true
        });
        this._bottomLabel = new St.Label({
            style_class: "geek-date-week-big-widget-bottom",
            x_expand: true
        });
    ...
    enable() {
        this._logger.info("geek-date-week-big enabled");
        this._render();
        this._setupTimer();
    }
    disable() {
        this._logger.info("geek-date-week-big disabled");
        this._destroyTimer();
    }
    getDefaultSettings() {
        return {
    ...
    _render() {
        ...
        const {family: dateFontFamily, size: dateFontSize} = _parseFontDescription(this._settings.dateFont ?? "Sans Bold 44", "Sans Bold", 44);
        const {family: weekFontFamily, size: weekFontSize} = _parseFontDescription(this._settings.weekFont ?? "Sans Bold 20", "Sans Bold", 20);
