// widgets/geek-week-date-bar/widget.js
//
// "Geek" week + date card - a wide `barx2` card (23×5 grid cells),
...
export default class GeekWeekDateBarWidget {
    /**
    ...
    buildActor() {
        this._actor = new St.BoxLayout({
            style_class: 'geek-week-date-bar-widget-root',
            vertical: true,
            x_align: Clutter.ActorAlign.CENTER,
        });

        this._topLabel = new St.Label({style_class: 'geek-week-date-bar-widget-top', x_expand: true});
        this._bottomLabel = new St.Label({style_class: 'geek-week-date-bar-widget-bottom', x_expand: true});
    ...
    enable() {
        this._logger.info('geek-week-date-bar enabled');
        this._render();
        this._setupTimer();
    }

    disable() {
        this._logger.info('geek-week-date-bar disabled');
        this._destroyTimer();
    }
    ...
    _render() {
        ...
        const {family: weekFontFamily, size: weekFontSize} =
            _parseFontDescription(this._settings.weekFont ?? 'Sans Bold 22', 'Sans Bold', 22);
        const {family: dateFontFamily, size: dateFontSize} =
            _parseFontDescription(this._settings.dateFont ?? 'Sans Bold 12', 'Sans Bold', 12);
