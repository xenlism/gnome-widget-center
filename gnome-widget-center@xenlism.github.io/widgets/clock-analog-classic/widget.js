...
export default class ClockAnalogClassicWidget {
    constructor(api) {
        this._api = api;
    ...
    buildActor() {
        this._layers = createLayeredCard({
            contentStyleClass: "clock-analog-classic-root"
        });
    ...
    _launchApp() {
        ...
        try {
            ...
            if (appInfo) appInfo.launch([], null); else this._api.logger.info(`clock-analog-classic: could not read .desktop file at ${path}`);
        } catch (e) {
            this._api.logger.info(`clock-analog-classic: failed to launch ${path}: ${e}`);
        }
    }
    ...
    _onRepaint() {
...

        {
            ...
            cr.setFontSize(size);
            this._setSourceHex(cr, s.numberColor, "#1A1A1AFF");
            const numberRadius = radius * 0.76;
            for (let i = 0; i < 12; i++) {
                const angle = i * (2 * Math.PI / 12) - Math.PI / 2;
                const x = cx + Math.cos(angle) * numberRadius;
                const y = cy + Math.sin(angle) * numberRadius;
                this._drawCenteredText(cr, HOUR_NUMERALS[i], x, y);
            }
        }
