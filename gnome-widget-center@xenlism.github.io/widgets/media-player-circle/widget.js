...
export default class MediaPlayerCircleWidget {
    ...
    _launchApp() {
        ...
        try {
            ...
            if (appInfo) appInfo.launch([], null); else this._api.logger.info(`media-player-circle: could not read .desktop file at ${path}`);
        } catch (e) {
    ...
    _render() {
        ...
        this._titleLabel.set_style(`color: ${infoColor}; font-family: ${infoFont.family}; font-size: ${infoFont.size}px; ` + `font-weight: bold; text-align: center;`);
        this._artistLabel.set_style(`color: ${infoColor}; font-family: ${infoFont.family}; font-size: ${Math.max(8, infoFont.size - 2)}px; ` + `opacity: 0.85; text-align: center;`);
        ...
        for (const button of [ this._prevButton, this._playPauseButton, this._nextButton ]) {
            button.set_style(`background-color: rgba(0,0,0,0.45); border-radius: 999px; padding: 7px; color: ${buttonColor};`);
    ...
    _renderState(state) {
        ...
        if (state.artUrl.length > 0) {
            try {
                ...
                this._artIcon.set_style(`border-radius: 999px; background-size: cover; background-position: center; ` + `background-image: url("${file.get_uri()}");`);
                this._showArt();
            } catch (e) {
                this._showFallbackArt();
            }
        ...
        if (state.status === "Playing") this._startTicker(); else this._stopTicker();
    }
    _startTicker() {
    ...
    _updateFraction(lengthMs) {
        const elapsedMs = (GLib.get_monotonic_time() - this._baseMonotonicUs) / 1e3;
