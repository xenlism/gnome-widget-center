...
export default class StickyNoteWidget {
    ...
    buildActor() {
        ...
        // Anchored the same way the color dot is anchored to the top-right
        // (expanding spacers pushing a fixed-size child to the far edge)
        // rather than relying on BinLayout end-alignment on a bare
        // DrawingArea, which was landing the fold near the middle of the
        // card instead of its corner.
        ...
        const foldRow = new St.BoxLayout({
            orientation: Clutter.Orientation.HORIZONTAL,
            x_expand: true
        });
        foldRow.add_child(new St.Widget({ x_expand: true }));
        foldRow.add_child(this._foldArea);

        const foldAnchor = new St.BoxLayout({
            orientation: Clutter.Orientation.VERTICAL,
            x_expand: true,
            y_expand: true
        });
        foldAnchor.add_child(new St.Widget({ y_expand: true }));
        foldAnchor.add_child(foldRow);

        this._stack.add_child(foldAnchor);
    ...
    _promptForNoteText(currentText) {
        return new Promise(resolve => {
            ...
            const entry = new St.Entry({
                ...
                x_expand: true,
                y_expand: true,
                style: "min-height: 180px; border-radius: 8px;"
            });
            // Multi-line: single_line_mode off + activatable off means Enter
            // inserts a newline instead of submitting - only the "Save"
            // button (or Escape to cancel) closes the dialog.
            const noteText = entry.clutter_text;
            noteText.set_single_line_mode(false);
            noteText.activatable = false;
            noteText.line_wrap = true;
            noteText.line_wrap_mode = Pango.WrapMode.WORD_CHAR;
            noteText.y_align = Clutter.ActorAlign.START;
            box.add_child(entry);
