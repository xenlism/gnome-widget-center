// widgets/mem-monitor/widget.js
//
// 1x1 card: big "NN% MEM" text on top, a scrolling sparkline history strip
...
// one across widgets would corrupt state shared with other metric widgets
// (this widget itself only reads getMemoryUsage(), which needs no delta).
//
...
export default class MemMonitorWidget {
    ...
    buildActor() {
        ...
        this._valueLabel = new St.Label({style_class: 'mem-monitor-value'});
        this._captionLabel = new St.Label({style_class: 'mem-monitor-caption'});
        this._textBox.add_child(this._valueLabel);
        this._textBox.add_child(this._captionLabel);

        this._graphArea = new St.DrawingArea({style_class: 'mem-monitor-graph', x_expand: true, height: GRAPH_HEIGHT});
    ...
    _launchApp() {
        ...
        try {
            ...
            if (appInfo)
                ...
                this._api.logger.info(`mem-monitor: could not read .desktop file at ${path}`);
        } catch (e) {
