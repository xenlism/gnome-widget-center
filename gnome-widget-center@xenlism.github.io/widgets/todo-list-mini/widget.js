    ...
    _promptForTaskText() {
        return new Promise(resolve => {
            ...
            const entry = new St.Entry({
                ...
                x_expand: true,
                y_expand: true,
                style: "min-height: 120px; border-radius: 8px;"
            });
            // Multi-line: single_line_mode off + activatable off means Enter
            // inserts a newline instead of submitting - only the "Add"
            // button (or Escape to cancel) closes the dialog.
            const taskText = entry.clutter_text;
            taskText.set_single_line_mode(false);
            taskText.activatable = false;
            taskText.line_wrap = true;
            taskText.line_wrap_mode = Pango.WrapMode.WORD_CHAR;
            taskText.y_align = Clutter.ActorAlign.START;
            box.add_child(entry);
