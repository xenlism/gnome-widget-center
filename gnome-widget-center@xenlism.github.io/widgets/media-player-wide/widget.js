...
export default class MediaPlayerWideWidget {
    ...
    _launchApp() {
        ...
        try {
            ...
            if (appInfo) appInfo.launch([], null); else this._api.logger.info(`media-player-wide: could not read .desktop file at ${path}`);
        } catch (e) {
    ...
    _render() {
        ...
        this._albumLabel.set_style(`color: ${infoColor}; font-family: ${infoFont.family}; font-size: ${Math.max(10, infoFont.size - 5)}px; opacity: 0.85;`);
        this._artistLabel.set_style(`color: ${infoColor}; font-family: ${infoFont.family}; font-size: ${Math.max(10, infoFont.size - 5)}px; opacity: 0.85;`);
        ...
        for (const button of [ this._prevButton, this._playPauseButton, this._nextButton ]) {
            button.set_style(`background-color: rgba(255,255,255,0.14); border-radius: 12px; padding: 10px; color: ${buttonColor};`);
    ...
    _renderState(state) {
        ...
        if (state.artUrl.length > 0) {
            try {
                ...
                this._artIcon.set_style(`border-radius: ${cornerRadius}px; background-size: cover; background-position: center; ` + `background-image: url("${file.get_uri()}");`);
                this._showArt();
            } catch (e) {
                this._showFallbackArt();
            }
