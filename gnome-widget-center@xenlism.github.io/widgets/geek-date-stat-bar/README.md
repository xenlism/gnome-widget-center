...
- **Date text** — font face/size (`dateFont`) and color (`dateColor`) for
  the big date line.
- **System text** — font face/size (`systemFont`) and color
  (`systemColor`) for the CPU/MEM/DISK line, plus how often it's
  re-sampled (`updateInterval`, seconds) and which filesystem path DISK
  measures (`diskPath`, default `/`).
...
- **Card** — background color (`backgroundColor`, transparency supported
  ...
  (`cornerRadius`).
- **Text Shadow** — optional shadow drawn under the date/system text
  itself (color, opacity, angle, distance, blur).
