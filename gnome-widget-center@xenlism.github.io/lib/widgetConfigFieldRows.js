// products/extension/lib/widgetConfigFieldRows.js
//
// Split out of widgetConfigUI.js (2026-08-01 lib/ cleanup pass) — the
// per-fieldType row builders (_textRow, _colorRow, _listRow, ...) that
// widgetConfigUI.js's `_buildRow()` dispatches to by `field.fieldType`.
// Moved here verbatim (same functions, same behavior) purely to bring
// widgetConfigUI.js down from ~1470 lines to a page-builder + condition-
// evaluator module; every export here is still only ever called from
// widgetConfigUI.js's `_buildRow()` switch statement.
//
// Prefs process ONLY (imports Adw/Gtk/Gdk/Pango/Gio/Soup) — same rule as
// widgetConfigUI.js itself (see that file's header).
...
function _getLocationSearchSession() {
    if (!_locationSearchSession)
        _locationSearchSession = new Soup.Session();
...
// `location` — a plain `text`-style "lat,lon" entry (same validation as
// _textRow() above) plus one built-in suffix icon button
// (`find-location-symbolic`, this codebase's standing convention for
// "auto-detect" - see _listRow()'s `item.format: "app"` scan button)
// that looks up the caller's approximate coordinates from their IP
// address and fills them straight into the entry.
//
// Replaces the older two-field "Place" (fieldType: "autocomplete", a
// per-widget autocomplete.js hitting Open-Meteo's geocoding API) +
// "Location" pair, and weather-modern's separate momentary "Detect
// automatically" switch field (see PROJECT_STATUS.md's 2026-07-29 and
// 2026-07-31 entries): both were extra fields with their own copy of the
// IP-lookup logic; this is one field, one button, and the lookup itself
// lives once in this file (_fetchIpLocationForPrefs() below) so every
// widget that declares `fieldType: "location"` gets identical UI/
// behavior for free, with no widget-specific autocomplete.js needed.
// `location` — a plain `text`-style "lat,lon" entry (same validation as
// _textRow() above) plus one built-in suffix icon button
// (`find-location-symbolic`, this codebase's standing convention for
// "auto-detect" - see _listRow()'s `item.format: "app"` scan button)
...
export function _locationRow(field, current, set) {
    ...
    editButton.connect('clicked', () => {
        // จุดที่ 3: ตรวจสอบ root ก่อนเสมอ
        ...
        // จุดที่ 1: ใช้ Gtk.Window แทน Adw.Window
        ...
        // จุดที่ 4: ใช้ Gtk.Box แทนการยัด SearchEntry ลง HeaderBar
        ...
        // จุดที่ 2: ใช้ set_child() แทน set_content()
        ...
        const performSearch = (keyword) => {
            ...
            // คำแนะนำเพิ่มเติม: ใช้ Session ที่ Reuse ได้ (สมมติว่าเรียก _getLocationSearchSession() ที่ประกาศไว้ด้านนอก)
    ...
    // ---------------------------------------------------------
    // ปุ่ม Location (ดึงพิกัดจาก IP Address)
    // ---------------------------------------------------------
        ...
        _fetchIpLocationForPrefs().then(coords => {
            if (!coords) {
                ...
                row.set_tooltip_text(
                    'Could not detect your location - check your connection, or type coordinates directly.');
...
function _getIpLookupSession() {
    if (!_ipLookupSession)
        _ipLookupSession = new Soup.Session();
    return _ipLookupSession;
}

/** @private shared fetch, same pattern as widget.js's own _fetchJson(). */
...
/**
 * Backs every `fieldType: "location"` row's suffix button. Tries, in
 * order: ip-api.com, freeipapi.com, ipwhois.io - the same free, no-API-
 * key IP-geolocation endpoints (and fallback order) each weather
 * widget's own `widget.js` already uses for its first-run auto-detect
 * default. Duplicated here rather than shared, since this file runs in
 * the prefs (GTK) process and a widget only ever imports its own files
 * (WIDGET_API.md §1) - there is no common module either side could
 * import from.
...
async function _fetchIpLocationForPrefs() {
    ...
    for (const {url, parse} of endpoints) {
        try {
            ...
            // Try the next endpoint - see widget.js's identical fallback loop.
        }
    }
    return null;
}
...
//
// Cross-field fill (a "selecting a Place updates Location too" style
// behavior): a search-result item may include a `fields` map
// ({siblingFieldId: value, ...}). Each entry is written to that
...
// ctx.registerRow() below (e.g. a "Place" field's search results
// including `fields: {location: "lat,lon"}`, and vice versa - no widget
// in this codebase currently does this; every weather widget's location
// field uses the simpler `fieldType: "location"` instead, see
// PROJECT_STATUS.md).
//
