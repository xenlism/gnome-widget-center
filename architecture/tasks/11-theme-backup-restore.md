...
GSettings/dconf (ดู `docs/SETTINGS_SPEC.md` "Host settings เอง") **ไม่ใช่ JSON file** — จึง
...
- `extension/lib/themeService.js` (สร้างใหม่ — `exportTheme(destPath)` / `importTheme(srcPath)`)
- `extension/prefs.js` (แก้เพิ่มปุ่ม "Export theme" / "Import theme" ใน Control Center —
