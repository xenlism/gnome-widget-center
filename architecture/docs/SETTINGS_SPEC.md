...
`extension/schemas/` ของ core เอง (ดู "Host settings เอง" ด้านล่าง) — ไม่มีไฟล์ `host.json`
...
## Host-side: `WidgetSettings` class (`extension/lib/widgetSettings.js`)
...
third-party widget — schema เดียว `extension/schemas/org.gnome.shell.extensions.widget-center.gschema.xml`
...
`extension/lib/settingsService.js` สำหรับตัวอย่างที่ถูกต้อง)
