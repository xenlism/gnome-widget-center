// widgets/clock-modern/prefs.js
//
// Prefs UI for the "Modern" vertical clock widget. Runs in the separate
// prefs (GTK4) process - does NOT import St/Clutter/Meta/Shell, same rule
// widgets/clock/prefs.js and widgets/calendar-modern/prefs.js follow.
...
export default class ClockModernWidgetPrefs {
    ...
    buildPrefsWidget() {
        const page = new Adw.PreferencesPage({title: 'Clock (Modern)'});

        const formatGroup = new Adw.PreferencesGroup({title: 'Format'});
        page.add(formatGroup);
        formatGroup.add(this._switchRow(
            'format24h', '24-hour format',
            'Off shows 12-hour time with an am/pm line above HH'
        ));
        ...
        launchGroup.add(this._switchRow(
            'launchOnClick', 'Launch app on click',
            'Requires an app to be chosen below'
        ));
    ...
    /**
     * @private
     * "Browse app .desktop" row - launchOnClick's target app. Uses
     * Gtk.FileDialog (same GTK 4.10+ dialog family as the
     * Gtk.ColorDialogButton rows above) filtered to .desktop entries, so
     * users pick a real installed app instead of typing a raw command.
     */
    _desktopFileRow() {
        ...
        browseButton.connect('clicked', () => {
            const dialog = new Gtk.FileDialog({title: 'Choose an application (.desktop file)'});
            ...
            // GTK app dirs are the natural starting point for browsing
            // installed .desktop files (~/.local/share/applications and
            // /usr/share/applications, in that order of preference).
            const appDirs = [
            ...
            if (startDir)
                dialog.set_initial_folder(Gio.File.new_for_path(startDir));
            ...
            dialog.open(parentWindow, null, (_dialog, result) => {
                try {
                    ...
                    // User cancelled the dialog, or the pick otherwise
                    // failed - nothing to persist either way.
                }
            });
        });
