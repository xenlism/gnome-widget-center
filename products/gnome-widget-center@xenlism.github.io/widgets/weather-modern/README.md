...
Same visual family as `weather-minimal`/`weather-dark`, but with three
things they don't have: a user-configurable refresh rate, an
alpha-enabled background color (translucent/transparent card), and a
"Detect automatically" switch that re-runs IP-based location lookup on
demand instead of only on first run.
...
`latitude,longitude` (e.g. `15.1111,85.2555`; spaces around the comma
are fine too). It's validated against that shape and saved as you type.

This intentionally matches weather-dark/weather-minimal's
`location`-only field rather than a "Place" (city-name) autocomplete
field - see `PROJECT_STATUS.md`'s 2026-07-29 entry: an earlier
autocomplete "Place" field only saved on `Adw.EntryRow`'s `apply`
signal (Enter / clicking a suggestion), so typing a city name and
tabbing away silently didn't persist anything. Until `widgetConfigUI.js`
gets a fixed autocomplete row (or a dedicated search-dialog field type),
every bundled weather widget sticks to `location` + IP auto-detect
instead of a broken "search by city name" field.
...
### Detect automatically (on demand)

Flip the "Detect automatically" switch in settings to re-run that same
IP lookup at any time, not just on first load - the switch flips itself
back off as soon as the lookup finishes (or fails), so it behaves like a
momentary button rather than a persistent setting.
...
|-------------------|------------|----------------|--------------------------------------------------------|
| `location`         | text       | `13.756331,100.501762` | `lat,lon` pair used for the weather fetch              |
| `detectNow`         | switch     | `false`        | Momentary: re-detects `location` from IP when turned on |
