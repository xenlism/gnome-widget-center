...
| Talks to a DBus service (media player, etc.) | `widgets/media-player/` + WIDGET_API.md §9.1 |
...
- `block-type: {cols, rows}` is in **grid cells** (16px/cell), not pixels.
  No field → `10 x 6` default. This is the widget's fixed size — there is
  no user-resize and no min/max.
...
- Don't add a `settings` array here unless you're doing step 3c below —
  `config.json` (3a), `settings` array (3c), and hand-written `prefs.js`
...
`config.json` instead — use one of those as your copy-paste starting
point.
...
these built generically, or any bundled widget's old git history for the
...
### 3c. `metadata.json`'s `settings` array — legacy, prefer 3a for new widgets
...
### 3d. `lib/settingsApi.js`'s `gwc.settings` builder — do NOT use this yet

It exists (`lib/settingsApi.js` + `lib/settingsRenderer.js` +
`lib/settingsStore.js` + the `widgets/_template/settings.js` example) and
has a richer type list than 3c (adds `date`, `action`, `multiOption`,
`showIf`), but **nothing in the real `extension.js`/`prefs.js` calls it.**
It's built against the legacy `imports.gi`/`imports.lib.*` syntax, not the
`import ... from 'gi://...'` ESM syntax this project's live host actually
uses — the only caller is `prefs/integration-example.js`, which says in
its own header it's *"not a real file the engine loads."* A widget built
against 3d today ships with no working settings UI at all. Full
explanation in WIDGET_API.md §6.3.
